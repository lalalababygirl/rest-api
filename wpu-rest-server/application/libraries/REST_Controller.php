    ...
    protected function _perform_ldap_auth($username = '', $password = NULL)
{
    ...
    if ($ldap['port'] < 1 || $ldap['port'] > 65535) {
        log_message('error', 'LDAP Auth: Port tidak valid: '.$ldap['port']);
        return FALSE;
    }

    log_message('debug', 'LDAP Auth: Connect to ' . $ldap['host']);
    ...
    if ($ldapconn) {
        ...
        log_message('error', 'LDAP Auth: Could not connect to LDAP server');
        return FALSE;
    }
